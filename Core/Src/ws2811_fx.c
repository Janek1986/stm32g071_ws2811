...
#define SEGMENT_LENGTH   (Ws28b11_Segments[mActualSegment].IdStop - Ws28b11_Segments[mActualSegment].IdStart + 1)
#define IS_REVERSE		Ws28b11_Segments[mActualSegment].Reverse
...
ws2811_color	mColor_w[NUM_COLORS];

typedef struct ws2811fx_s
{
	...
	ws2811_color	ModeColor_w[NUM_COLORS]; 	// Mode color struct representation
...
ws2811fx_s *Ws28b11_Segments = NULL;
...
uint8_t WS2811FX_GetSegmentsQuantity(void)
{
	return mSegments;
}

FX_STATUS WS2811FX_SegmentIncrease(void)
{
	if(mSegments < (WS2811_LEDS - 1))
	{
	 WS2811FX_Init(mSegments + 1);
	 return FX_OK;
	}
	return FX_ERROR;
}

FX_STATUS WS2811FX_SegmentDecrease(void)
{
	if(mSegments > 1)
	{
		WS2811FX_Init(mSegments - 1);
...
FX_STATUS WS2811FX_GetMode(uint16_t Segment, fx_mode *Mode)
{
	if(Segment >= mSegments) return FX_ERROR;
	*Mode = Ws28b11_Segments[Segment].ActualMode;
	return FX_OK;
}

FX_STATUS WS2811FX_NextMode(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	Ws28b11_Segments[Segment].CounterModeCall = 0;
	Ws28b11_Segments[Segment].CounterModeStep = 0;
	Ws28b11_Segments[Segment].ActualMode++;
	if(Ws28b11_Segments[Segment].ActualMode >= MODE_COUNT) Ws28b11_Segments[mActualSegment].ActualMode = 0;
	Ws28b11_Segments[Segment].mModeCallback = mMode[Ws28b11_Segments[mActualSegment].ActualMode];
	return FX_OK;
}

FX_STATUS WS2811FX_PrevMode(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	Ws28b11_Segments[Segment].CounterModeCall = 0;
	Ws28b11_Segments[Segment].CounterModeStep = 0;
	Ws28b11_Segments[Segment].ActualMode--;
	if(Ws28b11_Segments[Segment].ActualMode == 0) Ws28b11_Segments[mActualSegment].ActualMode = MODE_COUNT;
	Ws28b11_Segments[Segment].mModeCallback = mMode[Ws28b11_Segments[mActualSegment].ActualMode];
	return FX_OK;
}

FX_STATUS WS2811FX_SetReverse(uint16_t Segment, uint8_t Reverse)
{
	if(Segment >= mSegments) return FX_ERROR;
	WS2811FX_SetAll(Segment, BLACK); // Set all 'old' segment black

	if(Reverse > 1) Reverse = 1;

	Ws28b11_Segments[Segment].Reverse = Reverse;
	return FX_OK;
}

FX_STATUS WS2811FX_GetReverse(uint16_t Segment, uint8_t *Reverse)
{
	if(Segment >= mSegments) return FX_ERROR;
	*Reverse = Ws28b11_Segments[Segment].Reverse;
	return FX_OK;
}

FX_STATUS WS2811FX_SegmentIncreaseStart(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	WS2811FX_SetAll(Segment, BLACK); // Set all 'old' segment black

	if((Ws28b11_Segments[Segment].IdStop - Ws28b11_Segments[Segment].IdStart) > 0)
	{
		Ws28b11_Segments[Segment].IdStart++;
	}
	return FX_OK;
}

FX_STATUS WS2811FX_SegmentDecreaseStart(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	WS2811FX_SetAll(Segment, BLACK); // Set all 'old' segment black
	...
	if(Segment > 0)
	{
		if(Ws28b11_Segments[Segment-1].IdStop < (Ws28b11_Segments[Segment].IdStart - 1))
		{
			Ws28b11_Segments[Segment].IdStart--;
		}
	}
	...
	{
		if(0 < Ws28b11_Segments[Segment].IdStart)
		{
			Ws28b11_Segments[Segment].IdStart--;
		}
	}
...
FX_STATUS WS2811FX_SegmentIncreaseEnd(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	WS2811FX_SetAll(Segment, BLACK); // Set all 'old' segment black
	...
	{
		if(Ws28b11_Segments[Segment].IdStop < (Ws28b11_Segments[Segment+1].IdStart - 1))
		{
			Ws28b11_Segments[Segment].IdStop++;
		}

	}
	else // last Segment
	{
		if(Ws28b11_Segments[Segment].IdStop < (WS2811_LEDS - 1))
		{
			Ws28b11_Segments[Segment].IdStop++;
...
FX_STATUS WS2811FX_SegmentDecreaseEnd(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	WS2811FX_SetAll(Segment, BLACK); // Set all 'old' segment black
...
FX_STATUS WS2811FX_SetSegmentSize(uint16_t Segment, uint16_t Start, uint16_t Stop)
{
	if(Segment >= mSegments) return FX_ERROR;
	if(Start >= (WS2811_LEDS - 1)) return FX_ERROR;
	if(Stop >= (WS2811_LEDS - 1)) return FX_ERROR;
	if(Start > Stop) return FX_ERROR;

	WS2811FX_SetAll(Segment, BLACK); // Set all 'old' segment black

	Ws28b11_Segments[Segment].IdStart = Start;
	Ws28b11_Segments[Segment].IdStop = Stop;
	return FX_OK;
}

FX_STATUS WS2811FX_GetSegmentSize(uint16_t Segment, uint16_t *Start, uint16_t *Stop)
{
	if(Segment >= mSegments) return FX_ERROR;
	*Start = Ws28b11_Segments[Segment].IdStart;
...
uint8_t WS2811FX_IsAnyRunning(void)
{
	for(uint8_t i = 0; i < mSegments; i++)
	{
		if(Ws28b11_Segments[i].Running != 0)
			return 1;
	}
...
FX_STATUS WS2811FX_Stop(uint16_t Segment)
{
	if(Segment >= mSegments) return FX_ERROR;
	Ws28b11_Segments[Segment].Running = 0;
	if(!WS2811FX_IsAnyRunning())
		mRunning = 0;
	return FX_OK;
//	  strip_off();
}

FX_STATUS WS2811FX_IsRunning(uint16_t Segment, uint8_t *Running)
{
	if(Segment >= mSegments) return FX_ERROR;
	*Running = Ws28b11_Segments[Segment].Running;
	return FX_OK;
}

void WS2811FX_SetColorStruct(uint8_t id, ws2811_color c)
{
...
FX_STATUS WS2811FX_GetColorRGB(uint8_t id, uint8_t *r, uint8_t *g, uint8_t *b)
{
...
//	Value 		0-255
//
void WS2811FX_SetColorHSV(uint8_t id, uint16_t h, uint8_t s, uint8_t v)
{
	WS2811FX_HSVtoRGB(h, s, v, &mColor_w[id].red, &mColor_w[id].green, &mColor_w[id].blue);
	mColor[id] = ((mColor_w[id].red<<16)|(mColor_w[id].green<<8)|mColor_w[id]. blue);
}

void WS2811FX_SetColor(uint8_t id, uint32_t c)
{
	mColor[id] = c;
...
FX_STATUS WS2811FX_GetSpeed(uint16_t Segment, uint16_t *Speed)
{
	if(Segment >= mSegments) return FX_ERROR;
	*Speed = Ws28b11_Segments[Segment].Speed;
	return FX_OK;
}

FX_STATUS WS2811FX_IncreaseSpeed(uint16_t Segment, uint16_t Speed)
{
	return WS2811FX_SetSpeed(Segment, Ws28b11_Segments[mActualSegment].Speed + Speed);
}

FX_STATUS WS2811FX_DecreaseSpeed(uint16_t Segment, uint16_t Speed)
{
	return WS2811FX_SetSpeed(Segment, Ws28b11_Segments[mActualSegment].Speed - Speed);
}
...
void strip_off()
{
	for(uint16_t i = 0; i < WS2811_LEDS; i++)
	{
		WS2811_SetDiodeRGB(i, 0, 0, 0);
	}
	WS2811_Refresh();
}
...
// the old way, so many calls to the pokey getPixelColor() function made this super slow
  for(uint16_t i=0 + 1; i <WS2811_LEDS; i++) {
